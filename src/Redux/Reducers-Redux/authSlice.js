...
import {getFirestore,doc,collection,updateDoc,getDocs} from "@firebase/firestore";
...
export const signUp=createAsyncThunk("auth/signUp",async function (details){
    ...
    try {
      ...
      JSON.stringify(user ? user : null)
           ...
           createUserDocument(user,{address:"",email:user.email,latitude:"",longitude:"",} ); 
return user;
    } catch (error) {
        console.log(error.response);
    }
    });
  export const getPumpUserData=createAsyncThunk("auth/getPumpUserData",async(user)=>{
      console.log("triggered");
      const userRef=collection(db,"users");
const data=await getDocs(userRef);
const userDataObj=data.docs.map(user=>({...user.data(),id:user.id})).filter(item=>item.id===user.uid)[0];
console.log(userDataObj);
    return userDataObj ; 
  });
export const updateThePumpData=createAsyncThunk("auth/createThePumpData",async(details)=>{
